...
function vowelCount(str){
   ...
   subStr.forEach(function(letter){
    const lowerCaseLetter = letter.toLowerCase();
    if(vowels.indexOf(lowerCaseLetter) !== -1){
        if(obj.hasOwnProperty(lowerCaseLetter)){
            obj[lowerCaseLetter]++;
        }else{
...
function findInObj(arr, key, searchValue) {
    const res = arr.filter(function(obj){
        if(obj[key]&&obj[key]===searchValue) return true;
        else return false;
    });
    return res[0];
}
...
function removeVowels(str) {
    const lowerCaseSplitStr = str.split("");
    const vowels = 'aeiou';
    const res = lowerCaseSplitStr.filter(function(letter){
